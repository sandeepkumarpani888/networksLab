        ...
        *temp=0;
        ...
        while(true){
            ...
            *temp=counter;
            ...
            if(recvfrom(s_timeout,(char *)&packet,sizeof(packet),0,(struct sockaddr *)&saddr,&fromlen)<0){
                ...
                if(counter+1==*temp2 && rtlp->rtlp_acknum==checker_&& rtlp->rtlp_type==3 && rtlp->rtlp_chksum==csum(packet)){
                    printf("ECHO RESPONSE FROM SERVER %d\n",*temp2);
                    ackrecv=true;
                    printf("SENDING ACKNOWLEDGENMENT TO SERVER\n");
                    rtlp->rtlp_type=7;
                    //CORRECTING EVERYTHING 
                    correct();
                    ip->saddr = saddr.sin_addr.s_addr;
        ...
        while(1){
            ...
            if(recvfrom(s,(char *)&packet,sizeof(packet),0,(struct sockaddr *)&saddr,&fromlen)<0){
                ...
                rtlp->rtlp_destport = htons(port_client);
                printf("%d  |  %d  |  %d  |  %d  |  %d  |   %d\n",rtlp->rtlp_srcport,rtlp->rtlp_destport,rtlp->rtlp_seqnum,rtlp->rtlp_acknum,rtlp->rtlp_chksum,rtlp->rtlp_type);
                if(rtlp->rtlp_acknum==checker && rtlp->rtlp_type==6){
                    break;
                }
                else{
                    continue;
